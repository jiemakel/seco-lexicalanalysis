...
public class CombinedLexicalAnalysisService extends HFSTLexicalAnalysisService {
	...
	@Override
	public String baseform(String string, Locale lang, boolean baseformSegments, boolean guessUnknown) {
		return baseform(string, lang, baseformSegments, guessUnknown, 1);
	}

	public String baseform(String string, Locale lang, boolean baseformSegments, boolean guessUnknown, int depth) {
		try {
			List<WordToResults> crc = analyze(string, lang, Collections.EMPTY_LIST, baseformSegments, guessUnknown, depth);
	...
	public List<WordToResults> analyze(String str, Locale lang, List<String> inflections, boolean baseformSegments, boolean guessUnknown, int depth) {
		if (!supportedLocales.contains(lang)) return super.analyze(str, lang, inflections, baseformSegments, guessUnknown);
	...
	public List<WordToResults> analyze(String str, Locale lang, List<String> inflections, boolean segments, boolean guessUnknown) {
		return analyze(str, lang, inflections, segments, guessUnknown, 2);
	}
	...
	public String inflect(String string, List<String> inflections, boolean segments, boolean baseform, boolean guessUnknown, Locale lang) {
		StringBuilder ret = new StringBuilder();
		for (WordToResults part : analyze(string, lang, inflections, false, guessUnknown, 0)) {
	...
	public static void main(String[] args) {
		...
		print(las.analyze("spårassa", new Locale("fi"),Collections.EMPTY_LIST,false,true,2));
		...
		System.out.println(las.analyze("baa", new Locale("fi"),Collections.EMPTY_LIST,false,true,2));
		System.out.println(las.analyze("twiittasi", new Locale("fi"),Collections.EMPTY_LIST,false,true,2));
		System.out.println(las.baseform("twiittasi", new Locale("fi"), false, true));
		System.out.println(las.baseform("Me Aleksander Kolmas, Jumalan Armosta, koko Venäjänmaan Keisari ja Itsevaltias, Puolanmaan Zsaari, Suomen Suuriruhtinas, y.m., y.m., y.m. Teemme tiettäväksi: Suomenmaan Valtiosäätyjen alamaisesta esityksestä tahdomme Me täten armosta vahvistaa seuraavan rikoslain Suomen Suuriruhtinaanmaalle, jonka voimaanpanemisesta, niinkuin myöskin rangaistusten täytäntöönpanosta erityinen asetus annetaan:", new Locale("fi"), false, true));
		System.out.println(las.baseform("twiittasin", new Locale("fi"), false, true));
		System.out.println(las.baseform("twiittasi", new Locale("fi"), false, true));
		System.out.println(las.baseform("hän twiittasi", new Locale("fi"), false, true));
		System.out.println(las.analyze("hän twiittasi", new Locale("fi"),Collections.EMPTY_LIST,false,true,2));
		System.out.println(las.analyze("kiipesin puuhun", new Locale("fi"),Collections.EMPTY_LIST,false,true,2));
		System.out.println(las.baseform("ulkoasiainministeriövaa'at soitti fagottia", new Locale("fi"),true, true));
		System.out.println(las.analyze("ulkoasiainministeriövaa'at 635. 635 sanomalehteä luin Suomessa", new Locale("fi"), Arrays.asList(new String[] { "V N Nom Sg", "A Pos Nom Pl", "Num Nom Pl", " N Prop Nom Sg", "N Nom Pl" }), true, true));
		System.out.println(las.baseform("635. 635 Helsingissä ulkoasiainministeriöstä vastaukset sanomalehdet varusteet komentosillat tietokannat tulosteet kriisipuhelimet kuin hyllyt", new Locale("fi"),true, true));
		...
		System.out.println(las.inflect("sanomalehteä luin Suomessa kolmannen valtakunnan punaisella Porvoon asemalla", Arrays.asList(new String[] { "V N Nom Sg", "A Pos Nom Pl", "Num Nom Pl", " N Prop Nom Sg", "N Nom Pl" }), true, true, true, new Locale("fi")));
		System.out.println(las.inflect("maatiaiskanan sanomalehteä luin Suomessa kolmannen valtakunnan punaisella Porvoon asemalla", Arrays.asList(new String[] { "V N Nom Sg", "A Pos Nom Pl", "Num Nom Pl", " N Prop Nom Sg", "N Nom Pl" }), false, false, false, new Locale("fi")));		
