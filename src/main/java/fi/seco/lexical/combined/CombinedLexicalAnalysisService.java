...
public class CombinedLexicalAnalysisService extends HFSTLexicalAnalysisService {
	...
	@Override
	public String baseform(String string, Locale lang, boolean baseformSegments) {
		return baseform(string, lang, baseformSegments, 1);
	}

	public String baseform(String string, Locale lang, boolean baseformSegments, int depth) {
		try {
			List<WordToResults> crc = analyze(string, lang, Collections.EMPTY_LIST, baseformSegments, depth);
	...
	public List<WordToResults> analyze(String str, Locale lang, List<String> inflections, boolean baseformSegments, int depth) {
		...
		for (String sentence : getSentenceDetector(lang).sentDetect(str)) {
			for (String word : t.tokenize(sentence)) {
				...
				if (r.isEmpty() && word.length()>=4) { // Fixed cutoff, don't guess words shorter than 4 chars.
	...
	public List<WordToResults> analyze(String str, Locale lang, List<String> inflections, boolean segments) {
		return analyze(str, lang, inflections, segments, 2);
	}

	@Override
	public String inflect(String string, List<String> inflections, boolean segments, boolean baseform, Locale lang) {
		StringBuilder ret = new StringBuilder();
		for (WordToResults part : analyze(string, lang, inflections, false, 0)) {
	...
	public static void main(String[] args) {
		...
		System.out.println(las.baseform("twiittasi", new Locale("fi"), false));
		System.out.println(las.baseform("hän twiittasi", new Locale("fi"), false));
		System.out.println(las.analyze("hän twiittasi", new Locale("fi"),Collections.EMPTY_LIST,false,2));
		...
		System.out.println(las.inflect("sanomalehteä luin Suomessa kolmannen valtakunnan punaisella Porvoon asemalla", Arrays.asList(new String[] { "V N Nom Sg", "A Pos Nom Pl", "Num Nom Pl", " N Prop Nom Sg", "N Nom Pl" }), true, true, new Locale("fi")));
		System.out.println(las.inflect("maatiaiskanan sanomalehteä luin Suomessa kolmannen valtakunnan punaisella Porvoon asemalla", Arrays.asList(new String[] { "V N Nom Sg", "A Pos Nom Pl", "Num Nom Pl", " N Prop Nom Sg", "N Nom Pl" }), false, false, new Locale("fi")));		
